...
def user_advances_portal(request):
    """بوابة الموظف + عرض الحالات (ويُمنع الدخول لو الشروط غير متحققة)."""
    # فترات السُلفة
    ...
    # ✅ الحاجز: لو الراتب صفر أو مفيش فترة مفتوحة → ما يفتحش الصفحة
    if salary_zero:
        #or not (can_first or can_second):
        #if salary_zero:
        messages.error(request, "غير مسموح بفتح صفحة السُلفة لأن راتبك الأساسي غير مُسجّل أو يساوي صفر.")
        #else:
            #messages.error(request, "غير مسموح بفتح صفحة السُلفة الآن: لا توجد فترة سُلفة مفتوحة حاليًا.")
        return redirect('/')  # لو عندك Dashboard برابط مُسمّى، بدّله هنا

    # باقي البيانات (لن تُنفّذ إلا لو الشرطين متحققين)
